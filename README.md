...
class pendulum_f(RK4_f):
    ...
    def deriv(self, x, u):
        th, dth = x.T
        dthdt = dth
        ddthdt = -g/(l*m)*torch.sin(th) + u - dth*self.gamma
...
nu = None # None = u is just a number
ny = 4    # 4 outputs
nx = 2    # 2 states
...
for line_search_start in [1.0]:
    import time
    t_start = time.time()
    u_sol = mpc.solve(x_init=x_init, T=T, u_inits=u_inits, \
        y_targets=y_targets, y_weight=y_weight, u_weight=u_weight, \
            plot=False, u_bounds=u_bounds, verbose=1, line_search_start=line_search_start)
    print('line_search_start=',line_search_start, 'time', time.time() - t_start)
